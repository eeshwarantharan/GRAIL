...
def new_scene_command(args: argparse.Namespace) -> None:
    """
    Download an OSM area and scaffold a ready-to-run gnss-vim-sim project.

    Usage
    -----
      gnss-vim-sim new-scene --lat 12.9906 --lon 80.2296 --name my_campus
      gnss-vim-sim new-scene --lat 48.8566 --lon 2.3522 --name paris_demo --radius 500
      gnss-vim-sim new-scene --lat 51.5 --lon -0.12 --name london --offline
    """
    from gnss_vim_sim.world.osm_builder import build_new_scene

    out_dir = Path(args.out).resolve() if args.out else Path.cwd() / args.name
    build_new_scene(
        lat=args.lat,
        lon=args.lon,
        name=args.name,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    new_scene = sub.add_parser(
        "new-scene",
        help="download an OSM area and scaffold a ready-to-run project",
        description=(
            "Downloads building footprints from OpenStreetMap for a given lat/lon, "
            "converts them to PLY meshes, and writes a pre-populated demo_config.json. "
            "Requires: pip install osmnx shapely trimesh"
        ),
    )
    new_scene.add_argument("--lat",     type=float, required=True,
                           help="Scene origin latitude  (WGS-84 decimal degrees)")
    new_scene.add_argument("--lon",     type=float, required=True,
                           help="Scene origin longitude (WGS-84 decimal degrees)")
    new_scene.add_argument("--name",    required=True,
                           help="Project name (used in config and output directory)")
    new_scene.add_argument("--out",     default=None,
                           help="Output directory; defaults to ./<name>/")
    new_scene.add_argument("--radius",  type=float, default=800.0,
                           help="Scene half-width in metres (default: 800)")
    new_scene.add_argument("--offline", action="store_true",
                           help="Skip OSM download; scaffold config only (add PLY files manually)")
    new_scene.set_defaults(func=new_scene_command)
