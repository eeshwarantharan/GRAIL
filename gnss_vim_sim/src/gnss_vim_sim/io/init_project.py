...
def init_project(out_dir: Path, *, force: bool = False) -> list[Path]:
    ...
    readme.write_text(
        ...
        "# Open the WebGL mission planner in your browser\n"
        "gnss-vim-sim studio --config demo_config.json --out runs/mission_studio.html\n\n"
        "# Run the full 4-EKF simulation (uses bundled GRAIL classifier automatically)\n"
        "gnss-vim-sim run --config demo_config.json --out runs/demo_run\n"
        "```\n\n"
        "## Use your own OSM scene\n\n"
        "```bash\n"
        "# Download OSM buildings and scaffold a new project (requires osmnx)\n"
        ...
        "The bundled GRAIL XGBoost classifier (AUC ≈ 0.999 on synthetic data) is\n"
        "loaded automatically. To use a custom model:\n\n"
        "```bash\n"
        "gnss-vim-sim run --config demo_config.json --model-checkpoint my_model.pkl\n"
        "```\n\n"
        "The model must expose `predict_proba(X)` returning shape (N, 2) where\n"
        "column 1 is P(GNSS altitude error > 3 m).\n"
    )
